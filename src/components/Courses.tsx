...
const Courses = () => {
  ...
  const courses = [
    {
      ...
      duration: "٤ ساعات",
      description:
        "تعلم كيف تحوّل تواصلك مع المريض إلى ثقة وفرصة، واكتشف أسرار البيع في المجال الصحي عبر تقنيات عملية لبناء العلاقات، معالجة الاعتراضات، وإغلاق الصفقات باحترافية عند تقديم أي خدمة طبية.",
      topics: [
        "مدخل إلى المبيعات الطبية",
        "علم النفس في المبيعات",
        "الذكاء العاطفي والتواصل الفعّال",
        "استراتيجيات الإغلاق",
        "التعامل مع الاعتراضات بأنواعها",
    ...
    },
    {
      ...
      description:
        "كورس شامل يغطي جميع جوانب إدارة العيادة السنية، من بناء البراند الشخصي إلى التخطيط المالي والقيادة. مصمم ليمنحك رؤية كاملة وأدوات عملية لتحويل عيادتك إلى مشروع متكامل يحقق النجاح والاستدامة.",
      topics: [
    ...
    },
    {
      ...
      duration: "٤ ساعات",
      description:
        "كورس تدريبي أونلاين شامل و عملي يساعدك في بناء حضور رقمي قوي، تطوير محتواك، وتحويل حسابك إلى أداة لجذب المرضى.",
      topics: [
    ...
    },
    {
      ...
      description:
        "كورس متخصص في تطوير مهارات القيادة وإدارة الفرق في المجال الطبي. تعلم كيف تبني فريقاً متعاوناً، تدير الصراعات بفعالية، وتقود عيادتك نحو النجاح المستدام من خلال تطبيق مبادئ القيادة الحديثة.",
      topics: [
  ...
  const nextSlide = () => {
    if (currentIndex < (totalPages - 1) * pageSize) {
  ...
  // Always render exactly 3 slots (pad with nulls)
  const slice = courses.slice(currentIndex, currentIndex + pageSize);
  const placeholdersCount = Math.max(0, pageSize - slice.length);
  const visibleCourses = [...slice, ...Array(placeholdersCount).fill(null)];

  // Arrow states
  const isLeftDisabled = currentIndex === 0;
  const isRightDisabled = currentIndex >= (totalPages - 1) * pageSize;

  // Correct current page (no reverse)
  const currentPage = Math.floor(totalPages - 1 - currentIndex / pageSize);
      ...
      <div className="container-padding">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <button
            ...
            className={`absolute left-4 top-1/2 transform -translate-y-1/2 z-10 p-3 rounded-full shadow-lg transition-all duration-250 active:scale-95 active:shadow-md ${
              isLeftDisabled
          ...
          <button
            ...
            className={`absolute right-4 top-1/2 transform -translate-y-1/2 z-10 p-3 rounded-full shadow-lg transition-all duration-250 active:scale-95 active:shadow-md ${
              isRightDisabled
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed opacity-50'
                : 'bg-white/90 hover:bg-white text-primary hover:shadow-xl'
            }`}
          ...
          {/* Courses Display */}
          {/* Force LTR so items fill from the left even on RTL pages */}
          <div
            dir="ltr"
            className={`grid grid-cols-1 gap-8 px-4 md:px-16 transition-all duration-250 ease-in-out ${
              slideDirection === 'left' 
                ? 'transform translate-x-8 opacity-80' 
            ...
            {visibleCourses.map((course, index) =>
              course ? (
                <div
                  ...
                  // Switch back to RTL for Arabic content inside cards
                  dir="rtl"
                  className="border-2 border-primary/20 rounded-xl p-6 flex flex-col h-full bg-background shadow-lg hover:shadow-xl transition-all duration-1000 ease-in-out"
                >
                ...
                // Invisible placeholder keeps the grid slot so order/alignment is stable
                <div
                  key={`ph-${currentIndex}-${index}`}
                  className="border-2 border-transparent rounded-xl p-6 invisible"
                />
              )
            )}
          </div>
          ...
          <div className="flex justify-center mt-8 gap-2">
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                key={i}
                onClick={() => {
                  const targetIndex = (totalPages - 1 - i) * pageSize;
